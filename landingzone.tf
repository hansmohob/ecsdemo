...
resource "aws_resourcegroups_group" "Terraform" {
  name        = format("%s%s%s%s", var.PrefixCode, "rgg", var.EnvCode, "demoall")
...
resource "aws_s3_bucket" "alblogs" {
  bucket_prefix = format("%s%s%s%s", var.PrefixCode, "sss", var.EnvCode, "alblogs")
  force_destroy = true

  tags = {
    Name      = format("%s%s%s%s", var.PrefixCode, "sss", var.EnvCode, "alblogs"),
    rtype     = "storage"
    codeblock = "lzbase"
  }
}
